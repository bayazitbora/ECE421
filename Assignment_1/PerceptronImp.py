...
def fit_perceptron(X_train, y_train, max_epochs=5000):
    ...
    "*** YOUR CODE HERE ***"
    w = np.zeros(shape=(col+1,)); # initialize to zero vector
    ...
    # In each epoch, we examine all datapoints one by one and update w if neccessary. 
    # If after update you have a new best E_in, then save the updated weight in
    # your pocket to return it at the end

    w_pocket = w; # stores best weight vector for return

    for i in range(max_epochs):
        for j in range(row):
            "*** YOUR CODE HERE ***"
            current_error = errorPer(X_aug, y_train, w)

            # save weights if new error is lowest so far
            if current_error < Ein_best:
                Ein_best = current_error
                w_pocket = w
                # exit training algorithm with w_pocket if error = 0 (training done)
                if current_error == 0:
                    return w_pocket

            # Note: We cannot use the provided pred() function because it will classify W*X=0 as +1
            prediction = np.sign(np.dot(X_aug[j], w))
            
            # update w if a point is misclassified
            if y_train[j] != prediction:
                w += y_train[j]*X_aug[j]

    # return best value of w_pocket after exhausting all epochs
    return w_pocket
...
def errorPer(X_train, y_train, w):
    ...
    "*** YOUR CODE HERE ***"
    # array of all predictions. may contain zero values if point lies directly on hyperplane
    predictions = np.sign(np.matmul(X_train, w))

    num_misclassifications = np.sum(predictions != y_train)

    return num_misclassifications / y_train.size
...
def confMatrix(X_train, y_train, w):
    ...
    "*** YOUR CODE HERE ***"
    N,d = X_train.shape
    confusion_matrix = np.zeros(shape=(2,2), dtype=np.uint32)

    # Augment X_train to have additional column of 1's to simplify the matrix multiplication
    X_aug = np.hstack((np.ones(shape=(N, 1)), X_train))

    # array of all predictions. may contain zero values if point lies directly on hyperplane
    predictions = np.sign(np.matmul(X_aug, w))
    ...
    for i in range(y_train.size):
        ...
        # false positive
        elif y_train[i] == -1 and (predictions[i] == +1 or predictions[i] == 0):
            confusion_matrix[0, 1] += 1
        # false negative
        elif y_train[i] == +1 and (predictions[i] == -1 or predictions[i] == 0):
            confusion_matrix[1, 0] += 1
        # true positive
        elif y_train[i] == +1 and predictions[i] == +1:
...
def test_SciKit(X_train, X_test, y_train, y_test):
    ...
    "*** YOUR CODE HERE ***"
    model = Perceptron(tol=1e3, max_iter=5000)
    ...
    "*** YOUR CODE HERE ***"
    return confusion_matrix(y_test, y_test_predictions)
